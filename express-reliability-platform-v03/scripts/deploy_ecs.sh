...
echo '=== Step 3: Create security group - firewall rules ==='
...
aws iam create-role --role-name ecsExecRole-v03 \
  --assume-role-policy-document "$TRUST" 2>/dev/null || true
aws iam attach-role-policy --role-name ecsExecRole-v03 \
...
for SVC in flask-api node-api web-ui; do
  PORT=5000; [ "$SVC" = "node-api" ] && PORT=3000; [ "$SVC" = "web-ui" ] && PORT=80
  ...
  aws ecs register-task-definition \
    --family ${SVC}-v03 --network-mode awsvpc \
