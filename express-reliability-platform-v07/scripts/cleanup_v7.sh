#!/bin/bash
# V7 teardown: uninstalls Helm releases (apps + monitoring), deletes the
# platform namespace, destroys the EKS cluster, drains and destroys V7's own
# bootstrap (S3 state bucket + DynamoDB lock table), and removes the EKS
# context from ~/.kube/config.
#
# Intentionally DOES NOT touch:
#   - V6's bootstrap S3 bucket or DynamoDB lock table.
#   - V6's ECR repositories or images (V8+ may still need them).
#
...
echo "=== V7 Cleanup (env: ${ENV}) ==="
echo "This destroys the ${ENV} EKS cluster and (if no other env state remains)"
echo 'the V7 bootstrap state bucket, lock table, and ECR repos. Other envs in'
echo 'this same V7 stack are kept intact. V6 bootstrap is also kept intact.'
echo

ACCOUNT_ID=$(aws sts get-caller-identity --query Account --output text 2>/dev/null)
STATE_BUCKET="reliability-platform-v07-tfstate-${ACCOUNT_ID}"
LOCK_TABLE="terraform-state-lock-v07"
CLUSTER_VPC_NAME="reliability-platform-v07-${ENV}-vpc"
...
echo "=== Step 3: Re-init EKS stack for ${ENV} against V7 bootstrap backend ==="
...
if [ -n "${ACCOUNT_ID}" ]; then
  terraform -chdir=platform/terraform/eks init \
    ...
    -backend-config="key=eks/v7/${ENV}/terraform.tfstate" >/dev/null 2>&1
fi
...
# With per-env state keys (eks/v7/<env>/terraform.tfstate), the bootstrap
...
if [ -n "${ACCOUNT_ID}" ] && \
  ...
  for OTHER in dev staging prod; do
    ...
    if aws s3api head-object --bucket "${STATE_BUCKET}" \
         --key "eks/v7/${OTHER}/terraform.tfstate" \
...
if ${OTHER_ENV_STATE_FOUND}; then
  ...
  echo "    Re-run cleanup_v7.sh against the remaining envs first if you want"
  echo "    a full V7 teardown."
fi

echo '=== Step 5: Drain V7 state bucket (versions + delete markers) ==='
...
echo '=== Step 6: Destroy V7 bootstrap (S3 bucket + DynamoDB lock table) ==='
if ${OTHER_ENV_STATE_FOUND}; then
...
if ${OTHER_ENV_STATE_FOUND}; then
  echo '--- V7 state bucket (kept: other envs still use it) ---'
  echo "  ${STATE_BUCKET}"
else
...
echo 'V6 bootstrap and ECR repos are intact for V8-V10.'
