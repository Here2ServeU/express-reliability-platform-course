...
# S3 bucket for storing V8 Terraform state files.
#
# Naming: ${project_name}-${version_suffix}-tfstate-${account_id}: deliberately
# version-suffixed so V5 (`reliability-platform-tfstate-...`), V6
# (`reliability-platform-v06-tfstate-...`), V7 (`reliability-platform-v07-tfstate-...`), V8 (`reliability-platform-v08-tfstate-...`),
# and any future versions can coexist on one AWS account. Different version,
# different bucket: no conflicts, no shared blast radius.
...
# with V6's `terraform-state-lock-v06` table or any other version's table.
